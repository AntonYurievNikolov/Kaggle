...
path_5="final_weights_step1.hdf5"
path_6="final_weights_step2.h5"
path_7="final_weights_step3.h5"
...
def Datagen_sizecheck_model(filenames, batch_size, size_detection_mode=True, is_train=True,random_crop=True):
  ...
  while True:
    for i in range(len(filenames)):
      ...
      if is_train:
        y.append(filenames[i][1]/crop_ratio)
      else:
...
model.load_weights('final_weights_step1.h5')
...
# Start training. This kernel runs only 20 epochs. Longer training can improve the accuracy.
...
def NMS(score,y_c,x_c,height,width,iou_thresh,merge_mode=False):
  ...
  while len(box_idx)>0:
    ...
    top=top[still_alive]#np.maximum(0,top[still_alive])
    left=left[still_alive]#=np.maximum(0,left[still_alive])
    bottom=bottom[still_alive]#np.minimum(pred_out_h,bottom[still_alive])
    right=right[still_alive]#np.minimum(pred_out_w,right[still_alive])
...
def draw_rectangle(box_and_score,img,color):
  ...
  for i in reversed(list(range(number_of_rect))):
    #predicted_class="letter"#class_names[int(box_and_score[i,0])]
    #score, top, left, bottom, right = box_and_score[i,1:]
    top, left, bottom, right = box_and_score[i,:]
    #top*=print_h/pred_out_h
    #left*=print_w/pred_out_w
    #bottom*=print_h/pred_out_h
    #right*=print_w/pred_out_w
...
for i in np.arange(0,5):
  ...
  #数多いからmaxpoolのが早そう…。maxpool後にnmsというのも
  #print("after NMS",len(box_and_score))
  if len(box_and_score)==0:
    continue
  #print(class_names[int(dict_cat[annotation_list_val[i][1][0][4]])])
  #print(class_names[int(box_and_score[0,0])])
  #print(box_and_score[0,1])
  #print("top",box_and_score[0,2])
  #print("left",box_and_score[0,3])
  #print("bottom",box_and_score[0,4])  
  #print("right",box_and_score[0,5])  
  #max_prob=np.argsort(np.max(predict.transpose(2,0,1).reshape((category_n+4),-1)[:category_n,:],axis=1))
  ...
  #print(class_names[j],np.max(heatmap_0),class_names[k],np.max(heatmap_1))
  #heatmap_flatten=heatmap.reshape(-1)
  #img_draw=Image.open(cv_list[i][0]).convert('RGB')
  print_w, print_h = img.size
...
for i in np.arange(0,5):
  ...
  if len(box_and_score_all)==0:
    print("no box found")
    continue
  true_boxes=train_list[i][1][:,1:]#c_x,c_y,width_height
  top=true_boxes[:,1:2]-true_boxes[:,3:4]/2
  left=true_boxes[:,0:1]-true_boxes[:,2:3]/2
  bottom=top+true_boxes[:,3:4]
  right=left+true_boxes[:,2:3]
...
model.load_weights('final_weights_step3.h5')
