...
# Many studies on **Keypoint based detector** are conducted these days. One of the well-known keypoint-based detectors is **CornerNet**. The concept of ConerNet is predicting the coner of the bounding boxes using heatmaps like semantic segmentation. This detector is relatively easy to implement since it **does not need multiple ankers** unlike other well-known one-stage detectors such as RCNN, YOLO and SSD. 
...
# 
# DeNAさんはじめとした、様々な日本語の記事でも勉強させていただいておりますので、この場を借りてお礼申し上げます。
...
# First of all, let's convert the input data into the labels for CenterNet.
# 
# CenterNet用に入力データを形成しておきます。
...
# I'd like to check the size of objects before creating the model. If the object size is too small, detector cannot find the target. 
# 
...
# 
# 図は、文字サイズ÷ピクチャサイズの比率の分布を示したものですが、ピクチャ毎に結構ばらついています。相対的な文字の大きさが大きくなったり小さくなったりするよりも統一されていた方が検出しやすいので、統一方法を考えます。ここでは、やや強引に、CNNに文字の平均サイズを算出してもらえるか試してみることにします。
...
# 
# ResNetっぽいモデルで試してみます。入力は1ページごとの画像。出力は文字とピクチャのサイズ比とします。
...
# 
# 本カーネルの主目的はstep2(CenterNet)なので、少し短めのエポックで切り上げます。
...
model.load_weights('../input/centernet/final_weights_step1.hdf5')
model.save_weights('final_weights_step1.hdf5')
...
# 
# イマイチですが、とりあえず使っておきます。なお、学習をもう少し続ければもう少しマトモにはなります。
...
# 
# 後述するCenterNetでは出力を128x128のヒートマップにします。検出できるのはせいぜい25x25くらいだと考えて、元の画像データの分割数を適当に決めます。
...
# ここからCenterNetになります。step1とのモデルの違いは
# * ランダムクロッピングの追加
...
# 
# 真ん中が1-0の訓練用ターゲットデータです。右側がそれにガウス分布をあてたもので、検出器が中心の近傍点を検出した場合のロスを低減するために使用します。
...
reduce_lr = ReduceLROnPlateau(monitor = 'val_loss', factor = 0.5, patience = 10, verbose = 1)
"""
#model.load_weights('final_weights_step1.hdf5',by_name=True, skip_mismatch=True)
...
# 
# とりあえず30epochほど計算しますが、多分もっと長い方がよいです。
...
model.load_weights('../input/centernet/final_weights_step2.h5')
...
# 
# 中心点のヒートマップの出力例です。なんとなく文字の中心が色濃くなっています。
...
# (元の論文では周囲8マスをmax poolingで処理する方法も提案されています)
...
# Step1で得た分割数を適当に使って、分割した画像に対してそれぞれCenterNetにかけて、得られたデータをすべてひっくるめてNMSに投げることにします。
...
# 
# 最後は文字の分類です。CenterNetの検出が本カーネル主題なので、かなりいい加減な感じです。ごめんなさい。
...
df_unicode_translation=pd.read_csv("../input/kuzushiji-recognition/unicode_translation.csv")
...
# 
# 10epochほどにしますが、学習率下げるなどしてもう少し頑張るほうがよいと思われます。
...
model.load_weights('../input/centernet/final_weights_step3.h5')
